...
def test_ingest(test_data_dir: Path,
    ...
    output_dir = test_temp_dir / 'ingest_output'
    output_dir.mkdir(parents=True, exist_ok=True)

    _, ds_hash = ingest(electron_dataset=str(electron_dataset),
    ...
    output_file = output_dir / f'{ds_hash}.parquet'
    assert output_file.exists(), "Ingested dataset does not exist."
    assert pd.read_parquet(output_file).shape[0] > 0, "Ingested dataset is empty or unreadable."
